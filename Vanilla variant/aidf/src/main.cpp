...
namespace {
	...
	{
		ActorList companionList;

		RE::ObjectLoadedEventSource::GetSingleton()->RegisterSink(ActorLoadHandler::GetSingleton(companionList));
		MenuOpenedEventSource::GetSingleton()->RegisterSink(ConversationStartHandler::GetSingleton(companionList));
	}
	...
	void gameStartHook(F4SE::MessagingInterface::Message* a_msg)
	{
		...
		switch (a_msg->type)
		{
		case F4SE::MessagingInterface::kGameDataReady: initializeSinks(); break;
		default: break;
		}
	}
}
